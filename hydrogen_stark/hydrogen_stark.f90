...
!---------------------------------------------------------------
    ...
    integer, parameter :: Nnm = Nnindex * (Nnindex + 1) * (2*Nnindex + 1) / 6
    ...
    !double precision, allocatable :: Htemp(:,:) !store off-diagonal integrated elements
    !double precision, allocatable :: Enm(:,:)
    real*8, allocatable :: Hnm(:,:)
    real*8, allocatable :: Htemp(:,:) !store off-diagonal integrated elements
    real*8, allocatable :: Enm(:,:)
    real (dp) :: Hprime, Hc
    real (dp) :: p_int(Nnindex**2, Nnindex**2), pmn_int
    
    real (dp) :: phi0_int((1+Nnindex)*Nnindex/2,(1+Nnindex)*Nnindex/2)
    ...
    integer, parameter :: nout = ntot * (ntot + 1) * (2*ntot + 1) / 6
    real (dp) :: ediff
    ...
    do nindex1=1,Nnindex,1  
        do l1=0,nindex1-1,1
            do nindex2=1,Nnindex,1  
                do l2=0,nindex2-1,1
                    n1 = nindex1-l1-1
                    n2 = nindex2-l2-1
                    nl1 = nindex1 * (nindex1 - 1) / 2 + l1 + 1
                    nl2 = nindex2 * (nindex2 - 1) / 2 + l2 + 1
                    perp = 0
                    norm = 0
        ...
        do m1 = -l1, l1, 1
            do l2 = 0, Nnindex - 1
                do m2 = -l2, l2, 1
                    lm1 = l1**2 + m1 + l1 + 1
                    lm2 = l2**2 + m2 + l2 + 1
                    if (l2 > l1) p_int(lm1,lm2)=p_int(lm2,lm1)
                end do
            end do
        end do
    end do
    ...
    !$OMP PARALLEL DO
    phi0_int = 0
    do nl1 = 1,(1+Nnindex)*Nnindex/2
        do nl2 = 1, (1+Nnindex)*Nnindex/2
            Hprime=0
	        do i=0,mesh,1
            ...
            do s2 = 1, Nnm
                ...
                if (m1 == m2) then
                    ...
                    !off-diagonal block matrix consideration
                        ...
                        Hnm(s1,s2) = Hnm(s1,s2) + Htemp(s1,s2) * Hc
                    end if
                end if
            end do
        end do
...
!---------------------------------------------------------------------
subroutine solve_sheq ( n, l, e, e_const, mesh, a0, dx, r, sqr, r2, y, plotnull )
!---------------------------------------------------------------------
  use materials
...
100 do kkk = 1,maxiter
     ...
     nodes = n
     y(0) = r(0)**(l+0.5)*(1.0_dp + va/e_const*r(0)/x2l2)
     y(1) = r(1)**(l+0.5)*(1.0_dp + va/e_const*r(1)/x2l2)
     !y(0) = Rnm(0)
     !y(1) = Rnm(1)
