...
integer meshold
integer n, m, i
integer mr,mrmin,mrmax
integer s
integer,parameter :: mm = 4, nn=5
integer b
...
call eg_initial
    ...
    ml: do m=-mm,mm,1
        mrmin=max(-2-ceiling(dble(m)/2.0),-2+ceiling(dble(m)/2.0))
        mrmax=min(2-ceiling(dble(m)/2.0),2+ceiling(dble(m)/2.0))
       mrl: do mr=mrmin,mrmax,1
           ...
           nl: do n=1,nn,1 ! read number of nodes (stop if nodes < 0)
                write(*,*)m,n
              sl:  do s=0,nn-1
                  ! scan magnetic field
                    Bb=Bbmin
            ...
            200                 deallocate ( r, sqr, r2, vpot, y )
                        ...
                        ! write out the eigenvalue energy             
                            er=beta*(2*s+abs(dble(mr))+dble(tau)*dble(mr)+1.0_dp)
                            !write (12,'(e20.10,e20.10)') Bb, En0*(e+er)+eg
                            write (13,'(e20.10,e20.10)') Bb, En0*(e+er)+eg 
