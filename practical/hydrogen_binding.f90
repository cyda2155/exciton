...
integer meshold
integer n, m, i
integer,parameter :: mm = 3, nn=5
integer b
integer,parameter :: bN = 500
integer :: plotnull
integer :: fix=1
...
character(len = 2) :: cTemp
    ...
    Bbmax= 100.0_dp
    dBb= (Bbmax-Bbmin)/bN
    open(unit=13,file='alleigenvalue.dat')
    ...
    do m=0,mm,1
        ...
        do n=1,nn,1 ! read number of nodes (stop if nodes < 0)
            ...
            do b=0,bN,1
                ...
                call solve_sheq (n, m, e, beta, mesh, dx, r, sqr, r2, vpot, zeta, y, plotnull, fix )
                !if (b == 0) then
                !    e0 = e
                !end if
                !if ( plotnull == 0 ) then
    ...
    200                 deallocate ( r, sqr, r2, vpot, y )
                ...
                write (12,'(e20.10,e20.10)') Bb, En0*(e-beta*(2*dble(n-1)+abs(dble(m))+1))
                write (13,'(e20.10,e20.10)') Bb, En0*(e-beta*(2*dble(n-1)+abs(dble(m))+1))
...
!---------------------------------------------------------------------
subroutine solve_sheq ( n, m, e, beta, mesh, dx, r, sqr, r2, vpot, zeta, y, plotnull, fix )
  !---------------------------------------------------------------------
  !
  ...
  implicit none
  real(dp), parameter :: eps0=1.0D-10
  real(dp) :: eps
  integer, intent(in) :: mesh, n, m, fix
  ...
  !elw = minval (sqlhf/r2(:)+vpot(:))
  eup = maxval(vpot(:)+beta**2.0_dp/4.0_dp*r2(:))
  elw = minval(vpot(:)+sqlhf/r2(:))
  etempup=eup
  etemplw=elw
...
100  do kkk = 1,maxiter
     ...
     ! f > 0 forbidden
     icl = -1
     f(0) = ddx12 *( sqlhf + beta**2.0D0 /4.0D0 * r2(0)**2 + r2(0) * (vpot(0) -e) )
     
     do i=1,mesh
        f(i) = ddx12 *( sqlhf + beta**2.0D0 /4.0D0 * r2(i)**2 + r2(i) * (vpot(i) -e) )
        !
