...
character(len = 2) :: cTemp
    ...
    200                 deallocate ( r, sqr, r2, vpot, y )
                ...
                write (12,'(e20.10,e20.10)') Bb, En0*e
                write (13,'(e20.10,e20.10)') Bb, En0*e
...
subroutine solve_sheq ( n, m, e, beta, mesh, dx, r, sqr, r2, vpot, zeta, y, plotnull, fix )
  ...
  !elw = minval (sqlhf/r2(:)+vpot(:))
  eup = maxval(vpot(:)+beta**2.0_dp/4.0_dp*r2(:)+tau*beta*dble(m))
  elw = minval(vpot(:)+sqlhf/r2(:)+tau*beta*dble(m))
  etempup=eup
  etemplw=elw
...
100  do kkk = 1,maxiter
     ...
     ! f > 0 forbidden
     icl = -1
     f(0) = ddx12 *( sqlhf + beta**2.0D0 /4.0D0 * r2(0)**2 + r2(0) * (vpot(0)+tau*beta*dble(m)-e) )
     
     do i=1,mesh
        f(i) = ddx12 *( sqlhf + beta**2.0D0 /4.0D0 * r2(i)**2 + r2(i) * (vpot(i)+tau*beta*dble(m)-e) )
        !
